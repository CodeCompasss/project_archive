...
export async function GET() {
  try {
    ...
    const projectsWithMembers: ProjectWithMembers[] = allProjects.reduce((acc: ProjectWithMembers[], row) => {
...
export async function POST(_req: NextRequest) {
  try {
    const { projectName, projectDescription, projectLink, createdAt, members, projectCategoryOptions, customDomain } = await _req.json();
    ...
    console.error('Error adding project:', (error as Error).message);
